...
MCP_DISABLE_OVERRIDES="$(
    gosu "$TARGET_USER" env \
        "CODEX_HOME=$CODEX_DIR" \
        python3 -I /usr/local/lib/cage/cage_core/codex_runtime.py \
        runtime-overrides \
        --codex-bin /home/codex/.npm-global/bin/codex \
        --codex-home "$CODEX_DIR" \
        --repo "$WORK_DIR" \
        --profile "${CAGE_CODEX_PROFILE:-}" \
        --selected-stdio-json "${CAGE_MCP_SERVERS:-}" \
        --selected-remote-json "${CAGE_REMOTE_MCP_SERVERS:-}"
)" || {
