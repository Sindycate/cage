...
- Runs `docker run` with `cap_drop ALL` followed by the capabilities currently needed for UID/GID remapping; AppArmor and seccomp are unconfined for bubblewrap compatibility, and `no-new-privileges` is not currently set. Treat the container as accidental-damage isolation rather than a hostile-code boundary.
  ...
  - **Codex:** private snapshot of the host Codex directory from the preset auth block (default `~/.codex` if omitted) read-only at `/host-codex` for optional `auth.json` and supported global configuration, `OPENAI_API_KEY` env var if set. The entrypoint allowlists user `config.toml`, profile `*.config.toml` files, global `AGENTS.md`/`AGENTS.override.md`, `hooks.json`, and `rules/`, plus explicitly governed credentials; per-repository sessions, history, SQLite indexes, logs, memories, and caches remain volume-local and must never be replaced from the shared host directory. MCP OAuth `.credentials.json` stays on the host under one shared broker. Per-launch local bearer capabilities restrict each client to its selected servers; MCP sessions remain separate. The broker owns the legacy lifetime lease, so older Cage sessions must finish before migration, while new cages attach concurrently. Login/logout borrow the broker's credential mutex. If the preset selects `skill_packs`, each selected skill directory is mounted read-only at `/host-agent-skills/<name>` and copied into `$HOME/.agents/skills` inside the container. If no `skill_packs` are selected, the selected auth block's host agents directory is mounted read-only at `/host-agents` and copied wholesale so globally-installed skills (`npx skills add … -g`) are visible inside the container
...
The shared host broker owns Codex MCP OAuth `.credentials.json`. Container,
Desktop and unmonitored host clients attach with independent local capabilities;
only credential operations serialize. The broker rereads the authoritative store
for each request, records an in-progress marker before refresh, and durably
commits rotated tokens before returning them to its forwarding transport. An
interrupted refresh requires login rather than replaying a possibly spent token.
Local capabilities never contain upstream refresh tokens. HTTP redirects are
rejected, resource/issuer/client/scope bindings are checked, and streaming HTTP
responses remain streamed. Upstream session identifiers are scoped to the local
client. Each gated session supplies its own authenticated Netgate route; ambient
proxy variables cannot bypass it. `--net off` starts no broker.

The private control socket connection owns the client's lifetime. Disconnect
revokes its routes without stopping other clients. The broker exits after five
idle seconds; targets fail closed when their broker connection is lost. A
private manifest allows other Cage processes to find the broker. Existing
exclusive leases are respected; no live credential takeover occurs. Run login
through Cage, and do not concurrently run direct Codex against this OAuth store.
...
- Codex runtime state is per-repository volume data. Host import is allowlisted to user/profile TOML configuration, global AGENTS guidance, hooks/rules, plus the separately governed `auth.json`; MCP `.credentials.json` stays host-owned and stale volume copies are removed. Never copy or replace shared-host `sessions`, `archived_sessions`, `history.jsonl`, SQLite state, logs, memories, goals, caches, or shell snapshots over a project volume. Keep the fail-closed name checks inside both import helpers and the complete real-Docker preservation fixture when changing this boundary
